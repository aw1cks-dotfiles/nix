  ...
  aw1cks.profiles.nixos.desktop-perf =
    ...
    in
    {
      ...
      options.aw1cks.desktop.highPerf = {
        ...
        irqbalance.extraArgs = lib.mkOption {
          type = lib.types.listOf lib.types.str;
          default = [ ];
          description = "Extra command-line arguments passed to irqbalance.";
        };
      ...
      config = {
        ...
        # DAMON_RECLAIM: module parameters need ordered writes; live updates
        # must be committed instead of re-enabling an already-running kdamond.
        systemd.services.damon-reclaim-tune = lib.mkIf cfg.damonReclaim.enable {
          ...
          script =
            ...
            ''
              ...
              wait_commit_inputs_idle() {
                attempts=0
                while [ "$(< ${p}/commit_inputs)" = "Y" ]; do
                  if [ "$attempts" -ge 50 ]; then
                    echo "timed out waiting for DAMON_RECLAIM commit_inputs" >&2
                    exit 1
                  fi
              ...
              if [ "$enabled" = "Y" ]; then
                if ! echo Y                > ${p}/commit_inputs 2>/dev/null; then
                  echo "DAMON_RECLAIM parameters could not be committed; another DAMON operation is busy, skipping" >&2
                  exit 0
                fi
                wait_commit_inputs_idle
              else
                if ! echo Y                > ${p}/enabled 2>/dev/null; then
                  echo "DAMON_RECLAIM could not be enabled; another DAMON context is busy, skipping" >&2
                  exit 0
                fi
              fi
            '';
        ...
        systemd.services.irqbalance.serviceConfig.ExecStart = lib.mkIf (cfg.irqbalance.extraArgs != [ ]) (
          lib.mkForce [
            ""
            (utils.escapeSystemdExecArgs (
              [
                (lib.getExe config.services.irqbalance.package)
                "--journal"
              ]
              ++ cfg.irqbalance.extraArgs
            ))
          ]
        );
